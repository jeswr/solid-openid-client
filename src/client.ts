...
import { createAuthedFetch } from "./authed-fetch.js";
import { DEFAULT_MAX_REPLAY_BODY_BYTES } from "./body-buffer.js";
import { generateDpopKeyPair, toCryptoKeyPair } from "./dpop.js";
import { hasSecret, normalizeScope, resolveIdentity, selectClientAuth } from "./identity.js";
import { adaptCustomFetch, callbackToUrl } from "./request-adapter.js";
import {
...
export async function createSolidOidcClient(
  ...
  // The DPoP-attaching authed fetch (the resource-leg adapter). It reads the CURRENT access token
  // live per request (so a proof always binds to the latest token, incl. after a refresh), builds
  // the RFC 9449 §4.2 `ath`-bound proof via @jeswr/solid-dpop, and retries once on a §8 DPoP-Nonce
  // challenge. See `authed-fetch.ts` for the full protocol + its security invariants.
